...
def main_page():
    with open("./log.csv", "r") as f:
        feedings = [x.split(",")[0] for x in f.readlines() if "success" in x]
        feedings = sorted(feedings)
    date = datetime.now().strftime("%Y%m%d")
    return render_template("main_page.html", feedings=feedings, date=date)

# To make things a bit trickier, I put the address at the current yyyymmdd date
@app.route("/kib/<int:today>")
def kib(today):
    if today != int(datetime.now().strftime("%Y%m%d")):
